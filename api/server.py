...
def getVideoInfo(videoID):
	...
	try:
		...
		return render_template(# Rendering videoInfo.html
		                        ...
		                        videoStreams=yt.streams.all(),#.order_by('resolution').desc().all(),# Getting streams with both audio and video objects and rearranging them according resolution
		                        audioStreams=yt.streams.all())#.order_by('filesize').desc().all())# Getting streams with only audio objects and rearranging them according filesize
	except Exception as e:
